...
string file_hyperparameters = "hyperparameters.txt";
...
hyperparameters read_hyperparameters_from_file(std::string filename) {
  hyperparameters params;

  std::ifstream file(filename);
  file >> params.N;
  file >> params.test_size;
  file >> params.input_size;
  file >> params.hidden_size;
  file >> params.batch_size;
  file >> params.num_epochs;
  file >> params.num_classes;
  file >> params.reg;
  file >> params.learning_rate;
  file >> params.debug;
  file.close();
...
TEST(gtestProfile, nsys1)
{
  ...
  // hparams = hyperparameters{60000, 10000, 784, 512, 3000, 1, 10, 1e-4, 1e-2, 1};
  hparams = read_hyperparameters_from_file(file_hyperparameters);
